...
python ./instruct/LoRASelect.py --codebook ./instruct/codebooks_Diverse/ --instruct ./playground/Instructions_Diverse/Grounding/test.json
...
if [ ! -n "$2" ] ;then
    MODELPATH='./checkpoints/Instruction/Only_Pretrain_1.5_CIA/Grounding/llava-1.5-7b-lora'
else
    MODELPATH=$2
fi
...
for IDX in $(seq 0 $((CHUNKS-1))); do
    CUDA_VISIBLE_DEVICES=${GPULIST[$IDX]} python -m llava.eval.CoIN.model_vqa_cc_instruction \
        ...
        --question-file ./playground/Instructions_Diverse/Grounding/test.json \
...
python -m llava.eval.CoIN.eval_grounding \
    --test-file ./playground/Instructions_Diverse/Grounding/test.json \
